...
def fit_exponential_decay(time, depth, window_size):
    ...
    while best_r_squared < REGRESSION_THRESHOLD and window_size > 1:
        ...
        for i in range(len(time) - window_size + 1):
            ...
            window_time_numeric = (window_time - window_time[0]) / np.timedelta64(1, 's')
            ...
            try:
                ...
                residuals = window_depth - exponential_decay(window_time_numeric, *params)
        ...
        if best_r_squared < REGRESSION_THRESHOLD:
            print(f"R-squared below threshold: {best_r_squared} for window size {window_size}.")
            window_size -= 60
